/**
 * DPF Dual View Renderer — "Wireframe + Solid Shadow"
 *
 * Two overlaid representations: a dark solid silhouette (alpha=0.15) provides
 * mass and grounding, while a bright wireframe overlay provides technical
 * structure lines. Plasma effects are vivid between the two layers.
 * Think: technical illustration with dark fill and bright edge lines.
 */
...
const PHASE_COLORS = {
  ...
  radial:     [1.0, 0.35, 0.08],
  mhd_radial: [1.0, 0.35, 0.08],
...
// ============================================================
// DUAL-LAYER DEVICE — solid shadow + wireframe overlay
// ============================================================

function buildDualDevice(scene, G) {
  // Solid shadow materials (dark, low alpha)
  const anodeSolidMat = new BABYLON.StandardMaterial("anodeSolid", scene);
  anodeSolidMat.diffuseColor = new BABYLON.Color3(0.12, 0.12, 0.12);
  anodeSolidMat.emissiveColor = new BABYLON.Color3(0.04, 0.03, 0.02);
  anodeSolidMat.specularColor = new BABYLON.Color3(0.05, 0.05, 0.05);
  anodeSolidMat.alpha = 0.15;
  anodeSolidMat.backFaceCulling = false;

  const steelSolidMat = new BABYLON.StandardMaterial("steelSolid", scene);
  steelSolidMat.diffuseColor = new BABYLON.Color3(0.10, 0.10, 0.12);
  steelSolidMat.emissiveColor = new BABYLON.Color3(0.03, 0.03, 0.04);
  steelSolidMat.specularColor = new BABYLON.Color3(0.05, 0.05, 0.05);
  steelSolidMat.alpha = 0.15;
  steelSolidMat.backFaceCulling = false;

  // Wireframe overlay materials (bright, medium alpha)
  const anodeWireMat = new BABYLON.StandardMaterial("anodeWire", scene);
  anodeWireMat.diffuseColor = new BABYLON.Color3(0.85, 0.65, 0.35);
  anodeWireMat.emissiveColor = new BABYLON.Color3(0.25, 0.18, 0.06);
  anodeWireMat.specularColor = new BABYLON.Color3(0.5, 0.4, 0.2);
  anodeWireMat.alpha = 0.5;
  anodeWireMat.wireframe = true;

  const steelWireMat = new BABYLON.StandardMaterial("steelWire", scene);
  steelWireMat.diffuseColor = new BABYLON.Color3(0.55, 0.55, 0.6);
  steelWireMat.emissiveColor = new BABYLON.Color3(0.12, 0.12, 0.15);
  steelWireMat.specularColor = new BABYLON.Color3(0.4, 0.4, 0.45);
  steelWireMat.alpha = 0.5;
  steelWireMat.wireframe = true;

  const ceramicMat = new BABYLON.StandardMaterial("ceramic", scene);
  ceramicMat.diffuseColor = new BABYLON.Color3(0.85, 0.82, 0.75);
  ceramicMat.emissiveColor = new BABYLON.Color3(0.06, 0.05, 0.04);
  ceramicMat.specularColor = new BABYLON.Color3(0.1, 0.1, 0.1);
  ceramicMat.alpha = 0.25;

  // Anode — solid shadow
  const anodeSolid = BABYLON.MeshBuilder.CreateCylinder("anodeSolid", {
    diameter: G.anode_radius * 2, height: G.anode_length,
    tessellation: 32, cap: BABYLON.Mesh.CAP_ALL,
  }, scene);
  anodeSolid.rotation.z = Math.PI / 2;
  anodeSolid.position.x = G.anode_length / 2;
  anodeSolid.material = anodeSolidMat;
  anodeSolid.renderingGroupId = 0;

  // Anode — wireframe overlay
  ...
  anode.material = anodeWireMat;
  anode.renderingGroupId = 0;

  // Cathode rods — dual layer
  const N_RODS = G.n_cathode_rods || 8;
  const cathodeRods = [];
  for (let i = 0; i < N_RODS; i++) {
    const angle = (i / N_RODS) * Math.PI * 2;
    const py = G.cathode_radius * Math.sin(angle);
    const pz = G.cathode_radius * Math.cos(angle);

    const rodSolid = BABYLON.MeshBuilder.CreateCylinder("rodSolid" + i, {
      diameter: G.cathode_radius * 0.04, height: G.anode_length * 1.05, tessellation: 6,
    }, scene);
    rodSolid.rotation.z = Math.PI / 2;
    rodSolid.position.set(G.anode_length / 2, py, pz);
    rodSolid.material = steelSolidMat;
    rodSolid.renderingGroupId = 0;

    const rodWire = BABYLON.MeshBuilder.CreateCylinder("rod" + i, {
      diameter: G.cathode_radius * 0.04, height: G.anode_length * 1.05, tessellation: 6,
    }, scene);
    rodWire.rotation.z = Math.PI / 2;
    rodWire.position.set(G.anode_length / 2, py, pz);
    rodWire.material = steelWireMat;
    rodWire.renderingGroupId = 0;
    cathodeRods.push(rodWire);
  }
  ...
  baseRing.material = steelWireMat;
  ...
  return { anode, anodeSolid, cathodeRods, insulator, anodeWireMat, steelWireMat, ceramicMat };
}

// ============================================================
// SHEATH TORUS — blue-white ring sweeping along z
// ============================================================
...
// ============================================================
// PLASMA TRAIL — dim blue glow behind sheath
// ============================================================
...
// ============================================================
// PINCH COLUMN — white-hot core + orange-red mantle + m=0 ripple
// ============================================================
...
function buildPinchColumn(scene, G) {
  ...
  coreMat.emissiveColor = new BABYLON.Color3(1, 0.97, 0.88);
  ...
  mantleMat.emissiveColor = new BABYLON.Color3(1.0, 0.35, 0.08);
  ...
  // Halo — soft glow around pinch, backside rendered
  const haloMat = new BABYLON.StandardMaterial("haloMat", scene);
  haloMat.emissiveColor = new BABYLON.Color3(0.4, 0.25, 0.6);
  haloMat.disableLighting = true;
  haloMat.alpha = 0;
  haloMat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  haloMat.backFaceCulling = false;

  const halo = BABYLON.MeshBuilder.CreateTube("pinchHalo", {
    path: path, radiusFunction: function(i) { return radii[i] * 2.5; },
    tessellation: 16, cap: BABYLON.Mesh.NO_CAP,
    sideOrientation: BABYLON.Mesh.DOUBLESIDE, updatable: true,
  }, scene);
  ...
  return { core, mantle, halo, coreMat, mantleMat, haloMat, radii, path, N };
}
...
// ============================================================
// B-FIELD TORI — pulsing blue rings
// ============================================================
...
function buildBFieldRings(scene, G) {
  ...
  ringMat.emissiveColor = new BABYLON.Color3(0.15, 0.35, 0.9);
  ...
  for (let i = 0; i < N_RINGS; i++) {
    const xPos = G.anode_length * (0.15 + 0.7 * i / (N_RINGS - 1));
    const rFrac = 0.5 + 0.3 * Math.sin(Math.PI * i / (N_RINGS - 1));
    const ring = BABYLON.MeshBuilder.CreateTorus("bRing" + i, {
      diameter: G.cathode_radius * rFrac * 2, thickness: G.cathode_radius * 0.015,
      tessellation: 32,
    }, scene);
  ...
  return { rings, mat: ringMat };
}

function buildFieldLines(scene, G) {
  const lines = [];
  for (let i = 0; i < 4; i++) {
    const angle = (i / 4) * Math.PI * 2, r = G.cathode_radius * 0.6, pts = [];
    for (let k = 0; k <= 16; k++) {
      const xf = k / 16, twist = angle + xf * Math.PI * 0.5;
      pts.push(new BABYLON.Vector3(G.anode_length * xf,
        r * Math.sin(twist) * (1 - xf * 0.3), r * Math.cos(twist) * (1 - xf * 0.3)));
    }
    const line = BABYLON.MeshBuilder.CreateLines("fieldLine" + i, { points: pts }, scene);
...
function buildGroundGrid(scene, G) {
  const ext = G.cathode_radius * 8, gridLines = [], n = 20, step = ext * 2 / n;
  const y = -G.cathode_radius * 1.8;
  for (let i = 0; i <= n; i++) {
    const p = -ext + i * step;
    gridLines.push([new BABYLON.Vector3(p, y, -ext), new BABYLON.Vector3(p, y, ext)]);
    gridLines.push([new BABYLON.Vector3(-ext, y, p), new BABYLON.Vector3(ext, y, p)]);
  }
  const grid = BABYLON.MeshBuilder.CreateLineSystem("groundGrid", { lines: gridLines }, scene);
  grid.color = new BABYLON.Color3(0.12, 0.12, 0.15);
  grid.renderingGroupId = 0;
  return grid;
}

// ============================================================
// PARTICLES — blue-white gradient, additive blend, 3000 capacity
// ============================================================
...
function buildParticles(scene, G) {
  ...
  ps.emitter = emitter;
  ps.createSphereEmitter(G.anode_radius * 0.4);
  ps.color1 = new BABYLON.Color4(0.7, 0.85, 1.0, 0.9);
  ps.color2 = new BABYLON.Color4(0.3, 0.5, 1.0, 0.7);
  ps.colorDead = new BABYLON.Color4(0.05, 0.08, 0.2, 0);
  ps.minSize = G.cathode_radius * 0.004;
  ps.maxSize = G.cathode_radius * 0.014;
  ps.minLifeTime = 0.12;
  ps.maxLifeTime = 0.55;
  ps.emitRate = 0;
  ps.gravity = new BABYLON.Vector3(0, 0, 0);
  ps.minEmitPower = G.cathode_radius * 0.6;
...
function buildPipeline(scene, cam) {
  ...
  const glowNames = new Set([
    "sheathDisk", "pinchCore", "pinchMantle", "pinchHalo",
    "beamCone", "plasmaTrail",
  ]);
  glowLayer.customEmissiveColorSelector = function(mesh, _s, _m, result) {
    ...
    } else if (mesh.name.startsWith("bRing")) {
      const ec = mesh.material.emissiveColor;
      result.set(ec.r, ec.g, ec.b, mesh.material.alpha || 0);
    } else {
      result.set(0, 0, 0, 0);
...
async function createDPFScene(canvas, data) {
  ...
  const cam = new BABYLON.ArcRotateCamera("cam",
    -Math.PI / 4, Math.PI / 3.2, G.cathode_radius * 9,
  ...
  // Lights — subtle, emissives dominate
  const key = new BABYLON.DirectionalLight("key", new BABYLON.Vector3(-1, -2, 1), scene);
  key.intensity = 0.65;
  key.diffuse = new BABYLON.Color3(0.9, 0.88, 0.85);
  const fill = new BABYLON.HemisphericLight("fill", new BABYLON.Vector3(0, 1, 0), scene);
  fill.intensity = 0.3;
  fill.diffuse = new BABYLON.Color3(0.75, 0.8, 0.9);
  fill.groundColor = new BABYLON.Color3(0.15, 0.15, 0.2);

  // Device (dual-layer)
  const dev = buildDualDevice(scene, G);
  ...
  const grid = buildGroundGrid(scene, G);
  ...
  function applyFrame(i) {
    ...
    if (Ifrac > 0.01) {
      ...
      const fresnelBoost = 0.15 + 0.15 * Math.sin(t * 12);
      sheath.mat.alpha = clamp01(Ifrac * (0.35 + fresnelBoost));
      sheath.mat.emissiveColor.set(
        col[0] * 0.8 + 0.2, col[1] * 0.8 + 0.2, col[2] * 0.8 + 0.2);
      if (isP) {
        const scaleYZ = Math.max(0.05, cr);
        sheath.disk.scaling.y = scaleYZ;
        sheath.disk.scaling.z = scaleYZ;
      } else {
    ...
    // === PLASMA TRAIL — dim glow behind sheath ===
    if (Ifrac > 0.02 && f.z > G.anode_length * 0.05) {
      ...
      trail.mat.alpha = clamp01(Ifrac * 0.05);
      trail.mat.emissiveColor.set(col[0] * 0.2, col[1] * 0.2, col[2] * 0.4);
    } else {
      trail.trail.isVisible = false;
    }

    // === B-FIELD TORI — pulse with current ===
    const showBRings = Ifrac > 0.05;
    const bPulse = 0.5 + 0.5 * Math.sin(t * 8);
    for (let bi = 0; bi < bField.rings.length; bi++) {
      const ring = bField.rings[bi];
      ring.isVisible = showBRings;
      if (showBRings) {
        const phase = bi * 0.4 + t * 3;
        const pulse = 0.5 + 0.5 * Math.sin(phase);
        ring.scaling.y = 0.9 + 0.2 * pulse;
        ring.scaling.z = 0.9 + 0.2 * pulse;
      }
    }
    bField.mat.alpha = showBRings ? clamp01(Ifrac * 0.2 * bPulse) : 0;
    ...
    if (showPinch && pI > 0.03) {
      ...
      pinch.halo.isVisible = true;
      const pinchR = Math.max(G.anode_radius * 0.01, cr * G.cathode_radius * 0.12);
      rippleAmp = f.phase === "post_pinch" ? 0.35 : (f.phase === "pinch" ? 0.08 : 0);
      ...
      for (let k = 0; k <= pinch.N; k++) {
        ...
        const ripple = rippleAmp * lr * Math.cos(6.28 * waveNum * zf + t * 3);
      ...
      BABYLON.MeshBuilder.CreateTube("pinchHalo", {
        path: pinch.path,
        radiusFunction: function(j) { return pinch.radii[j] * 2.5; },
        tessellation: 16, cap: BABYLON.Mesh.NO_CAP,
        sideOrientation: BABYLON.Mesh.DOUBLESIDE, instance: pinch.halo,
      });

      pinch.coreMat.alpha = clamp01(pI * 0.85);
      pinch.mantleMat.alpha = clamp01(pI * 0.2);
      pinch.haloMat.alpha = clamp01(pI * 0.06);

      if (pI > 0.6) {
        pinch.coreMat.emissiveColor.set(1.0, 0.97, 0.88);
      } else {
        pinch.coreMat.emissiveColor.set(0.5 + pI * 0.5, 0.45 + pI * 0.5, 0.25 + pI * 0.6);
      }
      pinch.mantleMat.emissiveColor.set(
        lerp(0.8, 1.0, pI), lerp(0.2, 0.35, pI), lerp(0.04, 0.08, pI));
      pinch.haloMat.emissiveColor.set(
        lerp(0.2, 0.4, pI), lerp(0.15, 0.25, pI), lerp(0.3, 0.6, pI));
    } else {
      pinch.core.isVisible = false;
      pinch.mantle.isVisible = false;
      pinch.halo.isVisible = false;
    }
    ...
    // === PARTICLES — blue-white sparks near pinch ===
    ...
    // === ANODE GLOW — thermal emission under compression ===
    if (pI > 0.4) {
      dev.anodeWireMat.emissiveColor.set(
        0.25 + (pI - 0.4) * 0.6, 0.18 + (pI - 0.4) * 0.2, 0.06);
    } else {
      dev.anodeWireMat.emissiveColor.set(0.25, 0.18, 0.06);
    }

    // === PIPELINE adjustments ===
    glowLayer.intensity = 0.25 + pI * 0.35;
    pipeline.bloomWeight = 0.12 + pI * 0.18;
  ...
  return {
    ...
    bRings: bField.rings, fieldLines: fieldLines,
