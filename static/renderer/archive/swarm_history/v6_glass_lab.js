/**
 * DPF Glass Laboratory Renderer — "Museum Glass Model"
 *
 * The device is made of GLASS: transparent anode cylinder, frosted insulator,
 * thin glass cathode rods. Fresnel edge highlights define structure without
 * opacity. Plasma effects glow beautifully through the glass enclosure.
 *
...
const PHASE_COLORS = {
  rundown:    [0.15, 0.50, 1.0],
  radial:     [0.10, 0.70, 0.65],
  mhd_radial: [0.10, 0.70, 0.65],
  reflected:  [0.20, 0.55, 0.80],
  pinch:      [0.90, 0.95, 1.00],
  post_pinch: [0.40, 0.50, 0.80],
};
...
function makeGlassMat(name, scene, tint, alpha, fresnelBias, fresnelPower) {
  const mat = new BABYLON.StandardMaterial(name, scene);
  mat.diffuseColor = new BABYLON.Color3(tint[0] * 0.3, tint[1] * 0.3, tint[2] * 0.3);
  mat.specularColor = new BABYLON.Color3(0.6, 0.6, 0.65);
  mat.specularPower = 96;
  mat.emissiveColor = new BABYLON.Color3(tint[0] * 0.08, tint[1] * 0.08, tint[2] * 0.08);
  mat.alpha = alpha;
  mat.backFaceCulling = false;
  mat.emissiveFresnelParameters = new BABYLON.FresnelParameters();
  mat.emissiveFresnelParameters.bias = fresnelBias;
  mat.emissiveFresnelParameters.power = fresnelPower;
  mat.emissiveFresnelParameters.leftColor = new BABYLON.Color3(tint[0] * 0.5, tint[1] * 0.5, tint[2] * 0.5);
  ...
  mat.opacityFresnelParameters.rightColor = new BABYLON.Color3(0.15, 0.15, 0.15);
  return mat;
}

// ============================================================
// GLASS DEVICE — transparent anode, cathode rods, frosted insulator
// ============================================================

function buildGlassDevice(scene, G) {
  const anodeMat = makeGlassMat("anodeGlass", scene, [0.65, 0.75, 0.85], 0.12, 0.1, 3);
  const rodMat = makeGlassMat("rodGlass", scene, [0.5, 0.55, 0.6], 0.12, 0.1, 3);
  const insMat = makeGlassMat("insGlass", scene, [0.8, 0.82, 0.9], 0.18, 0.15, 2.5);
  insMat.diffuseColor = new BABYLON.Color3(0.35, 0.35, 0.40);
...
// ============================================================
// SHEATH TORUS — vivid blue-green ring between electrodes
// ============================================================
...
function buildSheathDisk(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(0.1, 0.6, 0.55);
  ...
  mat.emissiveFresnelParameters.leftColor = new BABYLON.Color3(0.3, 0.9, 0.8);
  mat.emissiveFresnelParameters.rightColor = new BABYLON.Color3(0.05, 0.3, 0.25);
  disk.material = mat;
...
// ============================================================
// PLASMA TRAIL — soft blue glow behind sheath
// ============================================================
...
function buildGasGlow(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(0.06, 0.15, 0.35);
  mat.disableLighting = true;
  mat.alpha = 0;
  mat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  mat.backFaceCulling = false;
...
// ============================================================
// PINCH COLUMN — white core + green-blue mantle with m=0 ripple
// ============================================================
...
function buildPinchColumn(scene, G) {
  ...
  coreMat.emissiveColor = new BABYLON.Color3(1, 0.97, 0.92);
  ...
  mantleMat.emissiveColor = new BABYLON.Color3(0.15, 0.55, 0.50);
...
function buildBeamCone(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(0.3, 0.55, 1.0);
  mat.disableLighting = true;
  mat.alpha = 0;
  mat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  cone.material = mat;
...
// ============================================================
// PINCH HALO — soft glow sphere around pinch site
// ============================================================

function buildHalo(scene, G) {
  const sphere = BABYLON.MeshBuilder.CreateSphere("halo", { diameter: G.anode_radius * 0.8, segments: 16 }, scene);
  sphere.position.x = G.anode_length;
  const mat = new BABYLON.StandardMaterial("haloMat", scene);
  mat.emissiveColor = new BABYLON.Color3(0.12, 0.35, 0.30);
  mat.disableLighting = true;
  mat.alpha = 0;
  mat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  mat.backFaceCulling = false;
...
// ============================================================
// B-FIELD TORI — thin glass-like rings showing topology
// ============================================================
...
function buildBFieldRings(scene, G) {
  ...
  ringMat.emissiveColor = new BABYLON.Color3(0.15, 0.35, 0.7);
  ...
  for (let i = 0; i < nRings; i++) {
    const r = G.anode_radius * (0.3 + 0.5 * i / nRings);
    const torus = BABYLON.MeshBuilder.CreateTorus("bRing" + i, {
      diameter: r * 2, thickness: r * 0.06, tessellation: 32,
    }, scene);
    torus.rotation.z = Math.PI / 2;
    torus.position.x = G.anode_length;
...
// ============================================================
// PARTICLES — blue-green to white, subtle
// ============================================================

function buildParticles(scene, G) {
  const emitter = new BABYLON.AbstractMesh("psEmitter", scene);
  emitter.position.x = G.anode_length;
  const ps = new BABYLON.ParticleSystem("sparks", 2000, scene);
  ps.emitter = emitter;
  ps.createSphereEmitter(G.anode_radius * 0.25);
  ps.color1 = new BABYLON.Color4(0.2, 0.8, 0.7, 0.7);
  ps.color2 = new BABYLON.Color4(0.9, 0.95, 1.0, 0.5);
...
function buildGroundGrid(scene, G) {
  ...
  grid.color = new BABYLON.Color3(0.12, 0.14, 0.18);
  grid.alpha = 0.3;
...
function buildPipeline(scene, cam) {
  ...
  const glowNames = new Set([
    "sheathDisk", "pinchCore", "pinchMantle", "beamCone", "gasGlow", "halo",
...
async function createDPFScene(canvas, data) {
  ...
  key.intensity = 0.6;
  key.diffuse = new BABYLON.Color3(0.9, 0.92, 0.95);
  ...
  function applyFrame(i) {
    ...
    const col = PHASE_COLORS[f.phase] || [0.15, 0.50, 1.0];
    ...
    if (Ifrac > 0.02 && f.z > G.anode_length * 0.05) {
      ...
      gas.mat.emissiveColor.set(col[0] * 0.25, col[1] * 0.3, col[2] * 0.45);
    } else {
    ...
    if (showPinch && pI > 0.03) {
      ...
      if (pI > 0.6) {
        ...
        pinch.coreMat.emissiveColor.set(0.5 + pI * 0.5, 0.6 + pI * 0.35, 0.5 + pI * 0.4);
      }
      pinch.mantleMat.emissiveColor.set(
        lerp(0.1, 0.2, pI), lerp(0.35, 0.6, pI), lerp(0.3, 0.55, pI));
    } else {
    ...
    if (showPinch && pI > 0.15) {
      ...
      haloObj.mat.emissiveColor.set(
        lerp(0.08, 0.2, pI), lerp(0.25, 0.45, pI), lerp(0.2, 0.4, pI));
    } else {
    ...
    beam.mat.alpha = beam.cone.isVisible ? clamp01(pI * 0.3) : 0;

    // === B-FIELD TORI ===
    const showB = isP && pI > 0.1;
    for (let bi = 0; bi < bField.bRings.length; bi++) {
      bField.bRings[bi].isVisible = showB;
      if (showB) {
        bField.bRings[bi].position.x = G.anode_length;
        const bScale = cr * (0.5 + 0.5 * bi / bField.bRings.length);
        bField.bRings[bi].scaling.set(bScale, bScale, bScale);
      }
    }
    bField.ringMat.alpha = showB ? clamp01(pI * 0.15) : 0;

    // === PARTICLES ===
    if (showPinch && pI > 0.2) {
      parts.ps.emitRate = Math.round(pI * 500);
    ...
    if (pI > 0.3) {
      const gv = (pI - 0.3) * 0.15;
      dev.anodeMat.emissiveColor.set(0.08 + gv, 0.12 + gv * 1.2, 0.15 + gv);
    } else {
      dev.anodeMat.emissiveColor.set(0.08 * 0.65, 0.08 * 0.75, 0.08 * 0.85);
    }

    // === PIPELINE ===
    glowLayer.intensity = 0.25 + pI * 0.35;
    pipeline.bloomWeight = 0.15 + pI * 0.15;
