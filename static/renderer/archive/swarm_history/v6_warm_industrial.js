/**
 * DPF Renderer v6 — "Warm Industrial"
 *
 * Premium engineering visualization with warm amber/copper palette.
 * Semi-transparent device (alpha=0.25) — see plasma through solid glass-like
 * electrodes, not wireframe. Plasma effects use fire colors: amber sheath,
 * orange-red compression, white-hot pinch. Think Lockheed Martin demo reel.
 */
...
const PHASE_COLORS = {
  rundown:    [1.0, 0.75, 0.25],
  radial:     [1.0, 0.50, 0.10],
  mhd_radial: [1.0, 0.50, 0.10],
  reflected:  [1.0, 0.60, 0.15],
  pinch:      [1.0, 0.95, 0.80],
  post_pinch: [0.9, 0.40, 0.12],
};
...
// ============================================================
// WARM INDUSTRIAL DEVICE — semi-transparent solid geometry
// ============================================================
...
function buildDevice(scene, G) {
  ...
  copperMat.specularColor = new BABYLON.Color3(0.9, 0.7, 0.35);
  copperMat.emissiveColor = new BABYLON.Color3(0.06, 0.03, 0.01);
  copperMat.specularPower = 48;
  copperMat.alpha = 0.25;
  copperMat.backFaceCulling = false;

  const steelMat = new BABYLON.StandardMaterial("steel", scene);
  steelMat.diffuseColor = new BABYLON.Color3(0.48, 0.46, 0.50);
  steelMat.specularColor = new BABYLON.Color3(0.55, 0.55, 0.58);
  ...
  ceramicMat.diffuseColor = new BABYLON.Color3(0.90, 0.85, 0.72);
  ceramicMat.specularColor = new BABYLON.Color3(0.15, 0.12, 0.08);
  ...
  const N_RODS = G.n_cathode_rods || 12;
  ...
  for (let i = 0; i < N_RODS; i++) {
    ...
    const rod = BABYLON.MeshBuilder.CreateCylinder("rod" + i, {
      diameter: rodDiam, height: G.anode_length * 1.05, tessellation: 10,
    }, scene);
  ...
  const ringThk = (G.cathode_radius - G.anode_radius) * 0.16;
...
// ============================================================
// GROUND GRID — warm-toned engineering floor
// ============================================================

function buildGroundGrid(scene, G) {
  const grid = BABYLON.MeshBuilder.CreateGround("grid", {
    width: G.cathode_radius * 14, height: G.cathode_radius * 14,
    subdivisions: 1,
  }, scene);
  grid.position.y = -G.cathode_radius * 1.8;
  grid.position.x = G.anode_length / 2;
  const mat = new BABYLON.StandardMaterial("gridMat", scene);
  mat.diffuseColor = new BABYLON.Color3(0.08, 0.06, 0.05);
  ...
  const N = 20;
  const span = G.cathode_radius * 12;
  for (let i = -N; i <= N; i++) {
    const pos = (i / N) * span / 2;
    const h = BABYLON.MeshBuilder.CreateLines("gl" + i, {
      points: [
        new BABYLON.Vector3(G.anode_length / 2 - span / 2, -G.cathode_radius * 1.79, pos),
        new BABYLON.Vector3(G.anode_length / 2 + span / 2, -G.cathode_radius * 1.79, pos),
      ],
    }, scene);
    h.color = new BABYLON.Color3(0.18, 0.14, 0.10);
    h.alpha = 0.25;
    const v = BABYLON.MeshBuilder.CreateLines("gv" + i, {
      points: [
        new BABYLON.Vector3(pos + G.anode_length / 2, -G.cathode_radius * 1.79, -span / 2),
        new BABYLON.Vector3(pos + G.anode_length / 2, -G.cathode_radius * 1.79, span / 2),
      ],
    }, scene);
    v.color = new BABYLON.Color3(0.18, 0.14, 0.10);
    v.alpha = 0.25;
  }
}

// ============================================================
// SHEATH DISK — warm amber annular ring
// ============================================================
...
function buildSheathDisk(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(1.0, 0.75, 0.25);
...
// ============================================================
// PLASMA TRAIL — gentle warm glow behind the sheath
// ============================================================
...
function buildGasGlow(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(0.25, 0.15, 0.05);
  mat.disableLighting = true;
  mat.alpha = 0;
  mat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  mat.backFaceCulling = false;
...
// ============================================================
// PINCH COLUMN — white-hot core, orange mantle
// ============================================================
...
function buildPinchColumn(scene, G) {
  ...
  coreMat.emissiveColor = new BABYLON.Color3(1.0, 0.97, 0.88);
...
// ============================================================
// HALO — soft warm glow sphere around pinch
// ============================================================
...
function buildHalo(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(1.0, 0.65, 0.20);
  mat.disableLighting = true;
  mat.alpha = 0;
  mat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  mat.backFaceCulling = false;
...
function buildBeamCone(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(1.0, 0.85, 0.50);
  mat.disableLighting = true;
  mat.alpha = 0;
  mat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  cone.material = mat;
...
// ============================================================
// B-FIELD TORI — subtle copper rings showing field structure
// ============================================================
...
function buildBFieldTori(scene, G) {
  ...
  mat.emissiveColor = new BABYLON.Color3(0.6, 0.35, 0.15);
  mat.disableLighting = true;
  mat.alpha = 0;
  mat.alphaMode = BABYLON.Engine.ALPHA_ADD;
  mat.backFaceCulling = false;
  ...
  for (let i = 0; i < N_RINGS; i++) {
    ...
    const ring = BABYLON.MeshBuilder.CreateTorus("bRing" + i, {
      diameter: G.anode_radius * (0.6 + frac * 0.8),
      thickness: G.anode_radius * 0.012,
      tessellation: 32,
    }, scene);
    ring.rotation.z = Math.PI / 2;
    ring.position.x = G.anode_length * (0.7 + frac * 0.25);
    ring.material = mat;
...
// ============================================================
// PARTICLES — warm golden sparks
// ============================================================

function buildParticles(scene, G) {
  const emitter = new BABYLON.AbstractMesh("psEmitter", scene);
  emitter.position.x = G.anode_length;
  const ps = new BABYLON.ParticleSystem("sparks", 2500, scene);
  ps.emitter = emitter;
  ps.createSphereEmitter(G.anode_radius * 0.25);
  ps.color1 = new BABYLON.Color4(1.0, 0.85, 0.40, 0.9);
  ps.color2 = new BABYLON.Color4(1.0, 0.60, 0.15, 0.7);
  ps.colorDead = new BABYLON.Color4(0.4, 0.15, 0.02, 0);
...
function buildPipeline(scene, cam) {
  ...
  glowLayer.customEmissiveColorSelector = function(mesh, _s, _m, result) {
    if (glowNames.has(mesh.name) && mesh.material && mesh.material.emissiveColor) {
      ...
      const ec = mesh.material.emissiveColor;
      result.set(ec.r, ec.g, ec.b, (mesh.material.alpha || 0) * 0.5);
    } else {
      result.set(0, 0, 0, 0);
    }
  };
...
async function createDPFScene(canvas, data) {
  ...
  scene.clearColor = new BABYLON.Color4(0.10, 0.08, 0.06, 1);
  scene.setRenderingAutoClearDepthStencil(1, true, true, false);
  scene.setRenderingAutoClearDepthStencil(2, false, false, false);

  // Camera
  const cam = new BABYLON.ArcRotateCamera("cam",
    -Math.PI * 0.30, Math.PI * 0.30, G.cathode_radius * 8.5,
  ...
  back.intensity = 0.4;
  back.diffuse = new BABYLON.Color3(0.95, 0.88, 0.75);
  const fill = new BABYLON.HemisphericLight("fill", new BABYLON.Vector3(0, 1, 0), scene);
  fill.intensity = 0.4;
  fill.diffuse = new BABYLON.Color3(0.95, 0.90, 0.80);
  fill.groundColor = new BABYLON.Color3(0.20, 0.15, 0.10);
  ...
  function applyFrame(i) {
    ...
    const col = PHASE_COLORS[f.phase] || [1.0, 0.75, 0.25];
    let rippleAmp = 0;
    ...
    if (Ifrac > 0.02 && f.z > G.anode_length * 0.05) {
      ...
      gas.mat.emissiveColor.set(col[0] * 0.25, col[1] * 0.15, col[2] * 0.08);
    } else {
    ...
    if (showPinch && pI > 0.03) {
      ...
      for (let k = 0; k <= pinch.N; k++) {
        ...
        const ripple = rippleAmp * lr * Math.cos(6.28 * waveNum * zf + f.t * 3);
      ...
      pinch.mantleMat.alpha = clamp01(pI * 0.20);

      if (pI > 0.6) {
        pinch.coreMat.emissiveColor.set(1.0, 0.97, 0.88);
      } else {
        pinch.coreMat.emissiveColor.set(
          lerp(0.6, 1.0, pI), lerp(0.4, 0.97, pI), lerp(0.15, 0.88, pI));
      }
      pinch.mantleMat.emissiveColor.set(
        lerp(0.85, 1.0, pI), lerp(0.25, 0.45, pI), lerp(0.05, 0.10, pI));
    } else {
    ...
    if (showPinch && pI > 0.1) {
      ...
      haloObj.mat.emissiveColor.set(
        lerp(1.0, 1.0, pI), lerp(0.5, 0.65, pI), lerp(0.1, 0.20, pI));
    } else {
    ...
    // === B-FIELD TORI ===
    const showBField = isP && pI > 0.15;
    for (let bi = 0; bi < bField.bRings.length; bi++) {
      bField.bRings[bi].isVisible = showBField;
    }
    if (showBField) {
      bField.mat.alpha = clamp01(pI * 0.08);
      const drift = f.t * 0.5;
      for (let bi = 0; bi < bField.bRings.length; bi++) {
        bField.bRings[bi].rotation.x = drift + bi * 0.4;
      }
    }
    ...
    if (pI > 0.35) {
      dev.copperMat.emissiveColor.set(
        0.06 + (pI - 0.35) * 0.35, 0.03 + (pI - 0.35) * 0.12, 0.01);
    } else {
      dev.copperMat.emissiveColor.set(0.06, 0.03, 0.01);
    }
