/**
 * DPF Ghost Renderer — "Ghost Device with Glowing Physics"
 *
 * Transparent wireframe electrodes with luminous plasma physics visible inside.
 * Designed for investor/student presentations: clear, dramatic, holographic feel.
 *
 * Visual concept: X-ray engineering display. The device is a wireframe cage,
 * the physics (sheath, pinch, beam) glows through it like a hologram.
 */
...
// ============================================================
// GHOST DEVICE — wireframe electrodes you can see through
// ============================================================

// Build mesh from inline ASSET_DATA (no file loading, works in data: URI iframes)
function meshFromAsset(name, assetKey, scene, mat, scaling, rotation, position) {
  if (typeof ASSET_DATA === "undefined" || !ASSET_DATA[assetKey]) return null;
  var d = ASSET_DATA[assetKey];
...
function buildGhostDevice(scene, G) {
  // Materials
  ...
  anodeWireMat.emissiveColor = new BABYLON.Color3(0.15, 0.10, 0.04);
  anodeWireMat.specularColor = new BABYLON.Color3(0.4, 0.3, 0.15);
  anodeWireMat.alpha = 0.35;
  anodeWireMat.wireframe = true;

  const steelMat = new BABYLON.StandardMaterial("steelWire", scene);
  steelMat.diffuseColor = new BABYLON.Color3(0.5, 0.5, 0.55);
  steelMat.emissiveColor = new BABYLON.Color3(0.08, 0.08, 0.10);
  steelMat.specularColor = new BABYLON.Color3(0.3, 0.3, 0.35);
  steelMat.alpha = 0.5;
  ...
  ceramicMat.alpha = 0.3;

  // Anode from inline asset: radius=1, height=5, Z-up, centroid z=2.5
  var scaleR = G.anode_radius;
  var scaleH = G.anode_length / 5;
  var anode = meshFromAsset("anode", "anode", scene, anodeWireMat,
    [scaleH, scaleR, scaleR], [0, 0, Math.PI / 2], [G.anode_length / 2, 0, 0]);
  if (!anode) {
    // Fallback to primitive if asset data not loaded
    anode = BABYLON.MeshBuilder.CreateCylinder("anode", {
  ...
  // Cathode rods from inline asset: radius=0.05, height=5.25, Z-up
  const N_RODS = G.n_cathode_rods || 8;
  const cathodeRods = [];
  var rodScaleR = (G.cathode_radius * 0.03) / 0.05;
  var rodScaleH = (G.anode_length * 1.05) / 5.25;
  for (let i = 0; i < N_RODS; i++) {
    const angle = (i / N_RODS) * Math.PI * 2;
    var rod = meshFromAsset("rod" + i, "cathode_rod", scene, steelMat,
      [rodScaleH, rodScaleR, rodScaleR],
      [0, 0, Math.PI / 2],
      [G.anode_length / 2, G.cathode_radius * Math.sin(angle), G.cathode_radius * Math.cos(angle)]);
    if (!rod) {
      rod = BABYLON.MeshBuilder.CreateCylinder("rod" + i, {
        diameter: G.cathode_radius * 0.03, height: G.anode_length * 1.05, tessellation: 6,
      }, scene);
      rod.rotation.z = Math.PI / 2;
      rod.position.set(G.anode_length / 2, G.cathode_radius * Math.sin(angle), G.cathode_radius * Math.cos(angle));
  ...
  // End rings from inline asset: Blender torus, major_r=1.25, XZ-plane, Y thin (minor=0.25)
  const ringThk = G.cathode_radius * 0.025;
  var ringScaleXZ = G.cathode_radius / 1.25;
  var ringScaleY = ringThk / 0.25;
  var baseRing = meshFromAsset("cathodeBase", "end_ring", scene, steelMat,
    [ringScaleXZ, ringScaleY, ringScaleXZ], [0, 0, Math.PI / 2], [0, 0, 0]);
  if (!baseRing) {
    baseRing = BABYLON.MeshBuilder.CreateTorus("cathodeBase", {
  ...
  // Insulator from inline asset: radius=1.25, height=0.15, XY-plane (Z thin)
  const insThk = G.insulator_thickness || G.anode_radius * 0.15;
  const insOuterR = G.anode_radius + (G.cathode_radius - G.anode_radius) * 0.3;
  var insulator = meshFromAsset("insulator", "insulator", scene, ceramicMat,
    [insOuterR / 1.25, insOuterR / 1.25, insThk / 0.15],
    [0, 0, Math.PI / 2], [-insThk / 2, 0, 0]);
  if (!insulator) {
    insulator = BABYLON.MeshBuilder.CreateCylinder("insulator", {
  ...
  return { anode, cathodeRods, insulator, anodeWireMat, steelMat, ceramicMat };
}

// ============================================================
// GLOWING SHEATH DISK — flat annular ring sweeping along z
// ============================================================

function buildSheathDisk(scene, G) {
  // Annular disk from anode_radius to cathode_radius
  const paths = [];
  const nRadial = 12;
...
// ============================================================
// GAS GLOW — subtle volume fill behind the sheath
// ============================================================
...
// ============================================================
// PINCH COLUMN — bright core + dim mantle at axis
// ============================================================
...
function buildPinchColumn(scene, G) {
  ...
  mantleMat.emissiveColor = new BABYLON.Color3(1.0, 0.4, 0.1);
...
// ============================================================
// PARTICLES — subtle accent sparks
// ============================================================

function buildParticles(scene, G) {
  const emitter = new BABYLON.AbstractMesh("psEmitter", scene);
  emitter.position.x = G.anode_length;
  const ps = new BABYLON.ParticleSystem("sparks", 1500, scene);
  ps.emitter = emitter;
  ps.createSphereEmitter(G.anode_radius * 0.3);
  ps.color1 = new BABYLON.Color4(1, 0.8, 0.3, 0.8);
  ps.color2 = new BABYLON.Color4(0.4, 0.6, 1.0, 0.6);
  ps.colorDead = new BABYLON.Color4(0.1, 0.1, 0.3, 0);
  ps.minSize = G.cathode_radius * 0.005;
  ps.maxSize = G.cathode_radius * 0.015;
  ps.minLifeTime = 0.15;
  ps.maxLifeTime = 0.5;
  ps.emitRate = 0;
  ps.gravity = new BABYLON.Vector3(0, 0, 0);
  ps.minEmitPower = G.cathode_radius * 0.5;
  ps.maxEmitPower = G.cathode_radius * 2;
...
function buildPipeline(scene, cam) {
  ...
  const glowNames = new Set(["sheathDisk", "pinchCore", "pinchMantle", "beamCone", "gasGlow"]);
...
async function createDPFScene(canvas, data) {
  ...
  // Auto-orbit
  let autoOrbit = true, interacting = false, timeout = null;
  ...
  // Lights — subtle, let emissives dominate
  const key = new BABYLON.DirectionalLight("key", new BABYLON.Vector3(-1, -2, 1), scene);
  key.intensity = 0.8;
  key.diffuse = new BABYLON.Color3(0.95, 0.93, 0.9);
  const fill = new BABYLON.HemisphericLight("fill", new BABYLON.Vector3(0, 1, 0), scene);
  fill.intensity = 0.35;
  fill.diffuse = new BABYLON.Color3(0.8, 0.85, 0.95);
  fill.groundColor = new BABYLON.Color3(0.2, 0.2, 0.25);

  // Device (ghost wireframe)
  const dev = buildGhostDevice(scene, G);
  ...
  function applyFrame(i) {
    ...
    if (Ifrac > 0.01) {
      ...
      sheath.mat.alpha = clamp01(Ifrac * 0.3);
      sheath.mat.emissiveColor.set(col[0], col[1], col[2]);

      if (isP) {
        // Contract the disk inward
        const innerR = G.anode_radius * 0.05;
        const outerR = f.r;
        const fullInner = G.anode_radius;
        const fullOuter = G.cathode_radius;
        const scaleY = Math.max(0.05, (outerR - innerR) / (fullOuter - fullInner));
        sheath.disk.scaling.y = scaleY;
        sheath.disk.scaling.z = scaleY;
      } else {
    ...
    // === GAS GLOW — swept region behind sheath ===
    if (Ifrac > 0.02 && f.z > G.anode_length * 0.05) {
      ...
      gas.mat.alpha = clamp01(Ifrac * 0.04);
      gas.mat.emissiveColor.set(col[0] * 0.3, col[1] * 0.3, col[2] * 0.5);
    } else {
    ...
    if (showPinch && pI > 0.03) {
      ...
      pinch.coreMat.alpha = clamp01(pI * 0.8);
      pinch.mantleMat.alpha = clamp01(pI * 0.18);

      if (pI > 0.6) {
        pinch.coreMat.emissiveColor.set(1.0, 0.98, 0.9);
      } else {
        pinch.coreMat.emissiveColor.set(0.5 + pI * 0.5, 0.4 + pI * 0.5, 0.2 + pI * 0.6);
      }
      pinch.mantleMat.emissiveColor.set(
        lerp(0.8, 1.0, pI), lerp(0.2, 0.4, pI), lerp(0.05, 0.1, pI));
    } else {
    ...
    // === PARTICLES — sparks near pinch ===
    if (showPinch && pI > 0.2) {
      parts.ps.emitRate = Math.round(pI * 600);
    ...
    if (pI > 0.4) {
      dev.anodeWireMat.emissiveColor.set(
        0.15 + (pI - 0.4) * 0.5, 0.10 + (pI - 0.4) * 0.15, 0.04);
    } else {
      dev.anodeWireMat.emissiveColor.set(0.15, 0.10, 0.04);
    }

    // === PIPELINE adjustments ===
    glowLayer.intensity = 0.2 + pI * 0.3;
    pipeline.bloomWeight = 0.1 + pI * 0.15;

    return { f, isP, cr, pI, rippleAmp: 0 };
  }
