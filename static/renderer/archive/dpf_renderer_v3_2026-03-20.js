...
// No CDN env texture — data: URI iframes have null origin, all CDN requests fail.
// PBR materials render solid with direct + hemispheric lighting (proven in test harness).
...
async function initEngine(canvas) {
  ...
  // No CDN env texture. data: URI iframes have null origin — CDN always fails.
  // PBR renders solid with direct + hemispheric + back lighting (proven in test harness).
  // Env texture is intentionally null; lights provide all illumination.
...
function createLights(scene) {
  ...
  // Back fill for solid electrode appearance from all angles
  const backLight = new BABYLON.DirectionalLight("back",
    new BABYLON.Vector3(1, -1, -1), scene);
  backLight.intensity = 0.6;
  backLight.diffuse = new BABYLON.Color3(0.9, 0.92, 0.95);

  const fillLight = new BABYLON.HemisphericLight("fill",
    new BABYLON.Vector3(0, 1, 0), scene);
  fillLight.intensity = 0.5;
  fillLight.diffuse = new BABYLON.Color3(0.9, 0.92, 1.0);
  fillLight.groundColor = new BABYLON.Color3(0.4, 0.4, 0.45);

  return { keyLight, backLight, fillLight };
}
...
function createDevice(scene, G) {
  ...
  // Copper for anode — StandardMaterial works without env texture
  const copperMat = new BABYLON.StandardMaterial("copper", scene);
  copperMat.diffuseColor = new BABYLON.Color3(
    anodeSpec.albedo[0] * 0.7, anodeSpec.albedo[1] * 0.6, anodeSpec.albedo[2] * 0.5);
  ...
  // Steel for cathode rods — StandardMaterial works without env texture
  const steelMat = new BABYLON.StandardMaterial("steel", scene);
  steelMat.diffuseColor = new BABYLON.Color3(
    cathodeSpec.albedo[0] * 0.55, cathodeSpec.albedo[1] * 0.55, cathodeSpec.albedo[2] * 0.6);
  ...
  // Cathode base ring (connects rod bottoms) — prevents "floating" look
  const rodLen = G.anode_length * 1.05;
  const ringThickness = (G.cathode_radius - G.anode_radius) * 0.25;
  ...
  baseRing.position.x = -rodLen * 0.02;
  baseRing.material = steelMat;
  baseRing.renderingGroupId = 0;
  cathodeRods.push(baseRing);

  // Cathode top ring (connects rod tops)
  const topRing = baseRing.clone("cathodeTop");
  topRing.position.x = rodLen * 1.01;
  cathodeRods.push(topRing);

  // Insulator — StandardMaterial, matte ceramic look
  const ceramicMat = new BABYLON.StandardMaterial("ceramic", scene);
  ceramicMat.diffuseColor = new BABYLON.Color3(insColor[0], insColor[1], insColor[2]);
...
function createPlasma(scene, G, L) {
  ...
  // Sheath sits between anode and cathode with clearance to avoid clipping
  const sheathMidR = (G.anode_radius * 1.15 + G.cathode_radius * 0.85) / 2;
  const sheathTubeR = (G.cathode_radius * 0.85 - G.anode_radius * 1.15) / 2;
...
function createGrid(scene, G) {
  ...
  gridMat.emissiveColor = new BABYLON.Color3(0.12, 0.14, 0.16);
  gridMat.alpha = 0.85;
...
function createParticles(scene, G) {
  ...
  ps.addColorGradient(0.15, new BABYLON.Color4(0.2, 0.5, 1.0, 0.25));
  ps.addColorGradient(0.4, new BABYLON.Color4(0.4, 0.8, 1.0, 0.3));
  ps.addColorGradient(0.7, new BABYLON.Color4(0.9, 0.95, 1.0, 0.15));
...
function updatePipelineForPhase(ctx, pI) {
  ...
  glowLayer.intensity = 0.4 + pI * 0.6;
  pipeline.bloomWeight = 0.2 + pI * 0.3;
  pipeline.bloomThreshold = 0.85 - pI * 0.15;
  pipeline.imageProcessing.exposure = 1.05 - pI * 0.1;
}

function updateAnodeGlow(ctx, pI) {
  if (pI > 0.6) {
    ctx.copperMat.emissiveColor.set((pI - 0.6) * 0.5, (pI - 0.6) * 0.15, 0.01);
  } else {
    ctx.copperMat.emissiveColor.set(0, 0, 0);
  }
}
...
function updateParticles(ctx, f, isP, pI) {
  ...
  if (f.phase === "rundown") {
    ...
    ps.emitRate = useGPU ? Math.min(8000, (1500 * boost) | 0) : Math.min(2000, (300 * boost) | 0);
...
function updateCinematicEffects(ctx, f, isP, pI) {
  ...
  // Camera auto-zoom
  if (isP && pI > 0.3 && autoOrbit) {
    const targetRadius = G.cathode_radius * (6 + (1 - pI) * 4);
    cam.radius += (targetRadius - cam.radius) * 0.015;
  } else if (autoOrbit && !userInteracting) {
    const defaultRadius = G.cathode_radius * 10;
    cam.radius += (defaultRadius - cam.radius) * 0.01;
  }

  // Subtle exposure shift only
  pipeline.imageProcessing.exposure = 1.0 + pI * 0.1;
}
