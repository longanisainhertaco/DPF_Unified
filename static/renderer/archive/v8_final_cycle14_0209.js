...
function buildSheath(scene, G) {
  ...
  return { torus, mat, midR, tubeR, fresnel, isShaderMat };
}
...
function buildGrid(scene, G) {
  ...
  // Add brighter center cross
  ctx.strokeStyle = "rgba(80,85,100,0.9)";
  ctx.lineWidth = 2;
...
async function createDPFScene(canvas, data) {
  ...
  // "One line; huge visual lift" — research finding
  ...
  function applyFrame(i) {
    ...
    if (Ifrac > 0.05) {
      ...
      if (isP) {
        ...
        // During radial: converge toward axis
      ...
      parts.ps.color1 = new BABYLON.Color4(pc[0], pc[1], pc[2], 0.85);
    ...
    pipeline.bloomWeight = lerp(0.15, 0.55, pI);     // diffuse radiance ramps with pinch
    pipeline.bloomThreshold = lerp(0.75, 0.40, pI);  // lower threshold captures more glow at peak
    pipeline.bloomScale = lerp(0.5, 0.7, pI);        // wider spread at peak
    ...
    plasmaLight.intensity = Ifrac * 0.6 * pulse;
    if (isP) {
