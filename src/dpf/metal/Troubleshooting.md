...
- **Fix applied (v1, incomplete)**: Changed `curl_B_mps()` default `mu_0` parameter from `MU_0` to `1.0`. This made J HL-consistent but broke the Hall formula: `E_Hall = (J x B)/(n_e*e)` uses SI constants (e in Coulombs, m_i in kg), so HL J with SI constants gives wrong magnitude.
- **Fix applied (v2, FIXED)**: `apply_hall_mhd_mps()` now converts B from HL to SI (`B_SI = B_HL * sqrt(mu_0)`), computes `J = curl(B_SI)/mu_0` in SI, evaluates `E_Hall = (J x B_SI)/(n_e*e)` with all-SI quantities, then converts `dB` back to HL (`dB_HL = dB_SI / sqrt(mu_0)`). The `curl_B_mps()` default remains `mu_0=1.0` for callers that want HL J; the Hall function explicitly passes `mu_0=MU_0`. Resistive diffusion is unaffected (uses its own HL-consistent eta_eff = eta/mu_0).
