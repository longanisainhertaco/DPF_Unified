...
2. **Download both Bruzzone 2001 PDFs** (HTTP 200 confirmed) — 30 min. These
   ...
   bibliography (refs [352]/[353]). They close `CLOSURE-BLK-ANOM-001` on
   the quantitative-formula side.
3. **Register at LXCat and download D₂ datasets** — 1-2 h. Closes two
   blockers (`STARTUP-BVP-CH02` and `CLOSURE-BLK-D2-EN-001`) in a single
   acquisition.
4. **Download SRIM-2013 + run D-on-D₂ stopping tables** — 2-4 h. Functional
   substitute for ICRU 49 (`NEUTRON-BLK-002`).
5. **Download Munro 2012 OSTI report** — 1-2 h. Re-export of the Brysk 1973
   FWHM formula derivation (`NEUTRON-BLK-004`).
6. **PlasmaPy Braginskii cross-check** — 1-2 h. Use as second-witness against
   the render-verified Table 2 values for `CLOSURE-BLK-BRAG-001`.

**Total Sprint 5 acquisition effort:** ~6-12 hours focused work; ~$0 cost
across all six items. After this, only 7 of 13 V2 P1+P2 external
acquisitions remain (Hagstrum 1956, Voronov 1997, Janev-Smith 1993,
Itikawa-Mason 2005 primary, Schmidt 2002, Schmidt 2012 PRL, Davidson-Gladd
1975) — each behind institutional access or interlibrary loan.
...
- The structural Te/Ti gap — no DPF in any literature publishes
  pinch-phase spectroscopic Te/Ti.
- The 3D-runner-vs-acceptance-gate code gap — still requires code work, not
  data acquisition.

The acquisitions in this memo close **source-availability** gaps. Runtime
